...
def index(request):
    ...
    if request.method == "POST":
        ...
        gdf_municipalities = gpd.read_file(
            "./geojson/typology_municipalities_4326.geojson"
        )
        ...
        commune_a_intersecter_geom = gdf_communes[
            gdf_communes["Nom_CMN"] == commune_request_name
        ].geometry.buffer(1000)
        gdf_communes_geom = gdf_communes.geometry

        result_touches = gdf_communes_geom.intersects(
...
def typologie(request):
    ...
    colormap = StepColormap(
        colors, index=range(len(colors)), vmin=1, vmax=len(colors), caption="Color Map"
    )

    # with open('./geojson/typology_municipalities_4326.geojson', 'r', encoding='utf-8') as f:
    #    data_mun = json.load(f)
    ...
    m = m._repr_html_()
    context = {"m": m, "typo_list": typo_list}

...
def index(request):
    ...
    for elem in list_env:
        list_env_descr.append(indicators[indicators['var'] == elem]['var_descr'].iloc[0])
    list_env_descr_enum = enumerate(list_env_descr)

    list_demo = ["RP0014","RP1524","RP2564","RP65M","RP80M","RP0014_F","RP1524_F","RP2564_F","RP65M_F","RP80M_F","RP0014_M","RP1524_M","RP2564_M","RP65M_M","RP80M_M"]
    for elem in list_demo:
        list_demo_descr.append(indicators[indicators['var'] == elem]['var_descr'].iloc[0])
    list_demo_descr_enum = enumerate(list_demo_descr)

    list_accessibility = ["ER_TIME","DENTAL_TIME","PHARMA_TIME","AMBU_TIME","D_MEDIC","D_SCHOOL_O","D_SCHOOL_S","D_EDUC","D_SECURITY","N_ACC_PED","N_ACC_BIC","D_GROCERY","PLAYGRD_TIME","D_FOREST","D_LAKE","D_SPORT","D_SWIM","D_STOP_TOT","GREEN_SP","BLUE_SP","HEALTHCARE_INDEX"]
    for elem in list_accessibility:
        list_accessibility_descr.append(indicators[indicators['var'] == elem]['var_descr'].iloc[0])
    list_accessibility_descr_enum = enumerate(list_accessibility_descr)

    list_socio_eco = ["R_PLA","AVG_PPH","MEDREV","R_DIV_WID","R_FFB","R_NN_FRA","R_NN_POBL","R_DIS","R_UNEMP","SOC_ECO_INDEX"]
    #list_socio_eco = ["AVG_PPH","R_FFB","R_NN_FRA","R_NN_POBL","R_DIS","R_UNEMP"]

    for elem in list_socio_eco:
        list_socio_eco_descr.append(indicators[indicators['var'] == elem]['var_descr'].iloc[0])
    ...
    if request.method=="POST":
        ...
        [x_list, x_text, limitations_informations, unit, href] = select_var_limites(var_name, gdf, gdf_kept)
...
def add_base_layers(m, highlight_function, geojson_dir):
    ...
    folium.GeoJson(gdf_municipalities, 
        ...
        zoom_on_click=True,
        overlay=False,
        tooltip=folium.GeoJsonTooltip(fields=['MUN_NAME', 'GROUP_ID', 'shortname', 'PTOT'], aliases=['Nom de la commune','Classe de typologie', 'Description de la typologie', 'Popultation totale']),
        style_function= lambda feature:{
        'fillColor': colormap(feature['properties']['GROUP_ID']),
        'fillOpacity': 0.6,
        'weight':1,
...
def add_informations(df_test, x_list, mean_commune, gdf_kept, var_name, map_dict, x_text, unit, commune_name, href, var_name_descr):
...
def add_legend_limitations_text(fig, x_list, x_text, href, y):
    if href[0] != "":
        fig.add_annotation(x=x_list[1], y=1 , text=href[0]+x_text[0]+"""</b></a>""", font=dict(size=10),showarrow=False,)
    else:
        fig.add_annotation(x=x_list[1], y=1 , text=x_text[0], font=dict(size=10),showarrow=False,)
    
    if href[1] != "":
        fig.add_annotation(x=x_list[2], y=0 , text=href[1]+x_text[1]+"""</b></a>""", font=dict(size=10),showarrow=False,)
    else:
        fig.add_annotation(x=x_list[2], y=0 , text=x_text[1], font=dict(size=10),showarrow=False,)

    if href[2] != "":
        fig.add_annotation(x=x_list[3], y=1 , text=href[2]+x_text[2]+"""</b></a>""", font=dict(size=10),showarrow=False,)
    else:
        fig.add_annotation(x=x_list[3], y=1 , text=x_text[2], font=dict(size=10),showarrow=False,)

    if href[3] != "":
        fig.add_annotation(x=x_list[4], y=0 , text=href[3]+x_text[3]+"""</b></a>""", font=dict(size=10),showarrow=False,)
    else:
        fig.add_annotation(x=x_list[4], y=0 , text=x_text[3], font=dict(size=10),showarrow=False,)
